...
export default async function CatalogoCostosPage({
  ...
  const { supabase, profile, allowed, canEdit } = await getCatalogAccess();
  ...
  let items = (data ?? []) as CostItem[];

  const search = (params.q ?? "").trim().toLowerCase();
  const categoryFilter = (params.category ?? "TODAS").trim();
      ...
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-neutral-400">
                {canEdit
                  ? "Modo administrador: puedes agregar y modificar costos."
                  : "Modo solo lectura: puedes consultar costos, pero no modificarlos."}
              </p>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <NavButton href="/cotizaciones" label="Cotizaciones" />
              {profile?.role === "admin" && <NavButton href="/admin" label="Admin" />}
            </div>
          </div>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          ...
          <MetricCard
            ...
            description="Ocultos del cotizador"
          />

          <MetricCard
            title="Modo"
            value={canEdit ? "Edición" : "Lectura"}
            description={canEdit ? "Solo admin" : "Vendedor sin edición"}
          />
        </section>
        ...
        {!canEdit && (
          <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/10 px-5 py-4 text-sm font-semibold leading-6 text-yellow-100">
            Estás viendo el catálogo en modo consulta. Puedes revisar costos,
            SKUs y unidades, pero solo un administrador puede editar.
          </div>
        )}
          ...
          <form className="grid gap-4 md:grid-cols-12">
            <label className="grid gap-2 md:col-span-5">
              ...
              <input
                ...
                placeholder="SKU, nombre, categoría o nota..."
        ...
        <section className="rounded-3xl border border-neutral-800 bg-neutral-900">
          <SectionTitle
            ...
            description={`${items.length} registros visibles en cost_catalog.`}
          />
...
function EditableTable({ items }: { items: CostItem[] }) {
  return (
...
function ReadOnlyTable({ items }: { items: CostItem[] }) {
  return (
      ...
      <tbody>
        {items.map((item) => (
          <tr key={item.sku} className="border-b border-neutral-800">
            ...
            <td className="px-3 py-3 text-right align-top text-neutral-300">
              {item.sale_price === null ? "—" : money(Number(item.sale_price))}
            </td>
